...
良いカウンセラーと同じで、ユーザーが自分に向き合えるようになり、もうアプリが要らなくなることを目指す。**依存させない。卒業を歓迎する。**

機能・コピー・通知・料金、あらゆる設計判断で「これはユーザーの自立を助けるか、依存させるか」を基準にする。詳細は `docs/DECISIONS.md` の冒頭を参照。
...
- 1日の感情バランス図
- カレンダー（色で感情を可視化）
- ※ 端末内ルールベース計算。Claude不要・オフラインでも動く

**分析（Pro限定）**
- 今日の気づき（Claude Haikuが日本語で生成）
- 週次・月次パターン検出
- ※ Insights画面を開いた時に初回のみ生成・キャッシュ。何度開いても1日1回分のAPI呼び出しのみ
...
無料
├── 記録：無制限
├── レーダーチャート・感情バランス図（スコアのみ）
└── 過去7日分

Pro（¥600/月 or ¥4,800/年）
├── 今日の気づき（Claudeによる分析文）← ここが課金動機
├── 過去データ無制限
├── 週次・月次インサイト・パターン検出
└── 将来：グループ機能
```

年額 ¥4,800 = 月換算 ¥400（月額の約2/3、4ヶ月分無料相当）

**課金導線①：** 「今日の気づき」を見ようとした時 → Pro誘導（毎日の体験に直結）
**課金導線②：** 8日目に過去データ閲覧 → Pro誘導
